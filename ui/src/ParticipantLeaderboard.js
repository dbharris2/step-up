    ...
    for (let person of this.props.participantsInfo) {
      contestantRows.push(
        <tr key={person["id"]} className="ParticipantLeaderboard-table-row">
          ...
          <td key={1} className="ParticipantLeaderboard-cell"> <ProgressBar steps={person["average"]} tiers={averageTiers} /> </td>
          <td key={2} className="ParticipantLeaderboard-cell"> <ProgressBar steps={person["total"]} tiers={totalTiers} /> </td>
        </tr>
      );
    }
    return (
